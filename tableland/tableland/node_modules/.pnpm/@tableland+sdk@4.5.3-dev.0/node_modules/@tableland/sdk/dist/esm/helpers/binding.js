...
export const placeholderRegExp = /(?:"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`|\[(?:[^[\\]|\\.)*\])|(\?\d*|[:@$][a-zA-Z_]\w+)/gmu;
...
export function getParameters(...values) {
...
export function bindValues(sql, parameters) {
    ...
    const a = anon;
    const n = named ?? {};
    const replaced = sql.replace(placeholderRegExp, function (m, group) {
        if (group == null) {
            return m;
        }
        if (group === "?") {
