...
function wrapMiddlewares(middlewares, fetch) {
    const handler = async (index, url, init) => {
        ...
        return await current(url, init, async (nextUrl, nextInit) => await handler(index + 1, nextUrl, nextInit));
    };
    return async (url, init) => await handler(0, url, init);
}
...
function fetcher() {
    ...
    return {
        ...
        path: (path) => ({
            method: (method) => ({
                create: ((queryParams) => createFetch(async (payload, init) => await fetchUrl({
                    baseUrl: baseUrl ?? "",
                    path: path,
                    method: method,
                    queryParams: Object.keys(queryParams != null || {}),
                    payload,
