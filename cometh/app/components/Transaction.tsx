...
export function Transaction({ transactionSuccess, setTransactionSuccess }) {
  const { wallet, counterContract } = useWalletAuth();
  const [isTransactionLoading, setIsTransactionLoading] = useState(false);
  const [transactionSended, setTransactionSended] = useState(null);
  const [transactionResponse, setTransactionResponse] = useState(null);
  const [transactionFailure, setTransactionFailure] = useState(false);
  const [nftBalance, setNftBalance] = useState(0);

  function TransactionButton({ sendTestTransaction, isTransactionLoading }) {
    return (
      <button
